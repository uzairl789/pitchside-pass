...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#020604] text-white">
      ...
      <section className="relative">
        ...
        <div className="mx-auto grid min-h-[650px] max-w-[1400px] items-center gap-10 px-6 py-14 lg:grid-cols-[0.9fr_1.1fr] lg:px-10">
          ...
          <div>
            ...
            <h1 className="mt-7 text-[54px] font-black uppercase leading-[0.92] tracking-[-0.055em] sm:text-[72px] lg:text-[82px]">
              YOUR PASS TO
            ...
            <div className="mt-9 grid max-w-[560px] grid-cols-2 gap-y-5 border-t border-white/[0.08] pt-6 sm:grid-cols-4">
              <TrustMini icon="◇" label="Official ticket links" />
              <TrustMini icon="♙" label="Your own membership" />
              <TrustMini icon="▭" label="Secure Stripe payments" />
              <TrustMini icon="▦" label="Manage online" />
            </div>
          </div>

          {/* RIGHT - PHONE CENTERPIECE */}
          ...
          <div className="flex items-center justify-center">
            <div className="w-full max-w-[470px] rounded-2xl border border-white/[0.09] bg-[#0a1510] p-6">
              ...
              <div className="mt-6 space-y-3">
                <TicketRow block="N1404" availability="7 available" />
                <TicketRow block="W208" availability="9 available" />
                <TicketRow block="S123" availability="4 available" />
              </div>
      ...
      {/* HOW IT WORKS */}
      <section id="how-it-works" className="px-6 py-5 lg:px-10">
        <div className="mx-auto max-w-[1280px] rounded-2xl border border-white/[0.08] bg-[#07100c] p-8">
          <div className="text-center">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#39bd76]">
              HOW IT WORKS
      ...
      <section id="pricing" className="px-6 py-5 lg:px-10">
        <div className="mx-auto max-w-[1280px] rounded-2xl border border-white/[0.08] bg-[#07100c] p-8">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            <ComingSoonCard club="LFC PITCHSIDE PASS" />

            <div className="relative rounded-xl border border-[#2e915b] bg-[#09150f] p-7">
              <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#2f7f4e] px-4 py-1 text-[10px] font-bold">
                AVAILABLE NOW
              </div>
              ...
              <Link
                href="/login"
                className="mt-6 flex h-12 items-center justify-center rounded-md bg-[#2a8a52] text-sm font-bold"
              >
            ...
            <ComingSoonCard club="ARSENAL PITCHSIDE PASS" />
          </div>
        </div>
      </section>
      ...
      <footer className="mt-5 border-t border-white/[0.06]">
        <div className="mx-auto flex max-w-[1280px] flex-col items-center justify-between gap-5 px-6 py-8 text-xs text-neutral-500 sm:flex-row lg:px-10">
          <Image
            src="/pitchside-logo.png"
            alt="Pitchside Pass"
            width={150}
          ...
          <span>Instant alerts. Better chances. Face value.</span>
        </div>
      </footer>
    </main>
  );
...
function TrustMini({
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-xl text-[#39bd76]">{icon}</span>
  ...
  return (
    <div className="border-white/[0.07] p-7 md:border-r last:border-r-0">
      <div className="flex items-center gap-4">
        ...
        <span className="text-sm font-bold text-[#39bd76]">{number}</span>
      </div>

      <p className="mt-5 text-xs font-bold">{title}</p>

      <p className="mt-3 text-xs leading-5 text-neutral-500">{text}</p>
    </div>
  );
}
...
function Arrow() {
  return (
    <span className="hidden text-xl text-neutral-600 md:block">→</span>
  );
}
...
function ComingSoonCard({ club }: { club: string }) {
  return (
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.015] p-7 opacity-45">
      <p className="text-sm font-bold">{club}</p>

      <p className="mt-5 text-sm text-neutral-400">
        Ticket monitoring and
        <br />
        instant alerts.
      </p>
      ...
      <button
        disabled
        className="mt-12 flex h-12 w-full items-center justify-center rounded-md bg-white/[0.04] text-xs font-bold text-neutral-600"
      >
        COMING SOON
      </button>
    </div>
